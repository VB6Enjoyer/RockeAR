...
session_start();
  ...
  $bandaAleatoria = getRandomArtist();
	$bandasBuscadas = searchBy("bandas", $_GET['nombre'], "%{$_GET['buscar']}%")// Esta funcion recibe el nombre de la tabla(bandas o usuario) un tipo(nombre, genero y localidad) y lo que va a buscar
?>
	...
	<body style="background-color: lightblue; color: #ffffff" >
		...
		======================================-->
			<div class="buscados"> 
		 		...
		 		<?php foreach($bandasBuscadas as $artista): ?>
		 			<div>  
			 			<h3>
			 				<a href="artista.php?id=<?php echo $artista['id']; ?>" class="list-group-item list-group-item-action list-group-item-primary"><?php echo $artista['nombre']; ?></a>
			 			</h3>
		 			</div>
		 		<?php endforeach;?>
		 		<?php if(count($bandasBuscadas) == 0): ?> <!-- Esto es lo que muestra si no encuentra ninguna banda --> 
		 			<p style="font-size:18px"> 
						<b>No se han encontrado resultados.</b>
					</p><br><br>
				<?php endif;?>
		 		<p style="font-size:16px">¿No encontraste el artista que buscabas? Quizás podrías intentar <a href="agregar.php" class="btn btn-outline-light btn-sm active" role="button" aria-pressed="true"> Añadirlo a la página</a></p>
		 	</div>	
			<div class="center" id="busqueda-lista"></div>		
		<!--=====  End of LISTA BUSCADOS  ======-->
</body>
</html>